...
public class MainTest {
	...
	private static final String 
			...
			END_PROGRAM = "PROGRAM TERMINATED", 
			EMPTY_STRING = "", SPACE = " ",
			EXC_END_PROGRAM = "No line found", 
			EXC_EMPTY_INPUT = "ERROR: EMPTY INPUT",
			EXC_ON_CHAR = "Input error:  ",
			EXC_EXPECTED = "Expected: ",
			EOLN = "End of line",
			INVALID_OPERATOR = "Input error, invalid operator: ",
			LETTER = "Letter (A-Z,a-z)",
			NUMBER = "Natural umber",
	...
	private char additiveOperator(Scanner in) throws APException {
		if (nextCharIs(in, UNION)) {
			...
			throw new APException(INVALID_OPERATOR + in.next());
		}
	}
